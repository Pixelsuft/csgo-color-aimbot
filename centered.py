...
fov_del = int(fov / 2)
running = True
halfer = (int(monitor['width'] / 2), int(monitor['height'] / 2))
...
def checker(i, j):
    ...
    if color[0] - nt_del < r < color[0] + nt_del and color[1] - nt_del < g < color[1] + nt_del\
        ...
        ccc = img.getpixel((fov_del - 7, fov_del - 7))
...
while True:
    ...
    if in_game:
        ...
        if running:
            ...
            for i in range(halfer[0]):
                breaked = False
                for j in range(halfer[1]):
                    if checker(halfer[0] - i, halfer[1] + j) or checker(halfer[0] + i, halfer[1] + j) or\
                        checker(halfer[0] - i, halfer[1] - j) or checker(halfer[0] + i, halfer[1] - j):
                        breaked = True
                        break
                if breaked:
                    break
